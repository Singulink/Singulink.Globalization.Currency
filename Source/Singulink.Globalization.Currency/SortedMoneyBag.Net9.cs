...
/// <content>
/// Contains .NET 9+ implementations for MoneyBag.
/// </content>
partial class SortedMoneyBag
{
    ...
    public void Add(decimal amount, string currencyCode) => Add(amount, currencyCode.AsSpan());
    ...
    public bool Remove(string currencyCode) => Remove(currencyCode.AsSpan());

    /// <inheritdoc cref="SetAmount(decimal, TargetDependentStringKey)"/>
    public void SetAmount(decimal amount, string currencyCode) => SetAmount(amount, currencyCode.AsSpan());

    /// <inheritdoc cref="Subtract(decimal, TargetDependentStringKey)"/>
    public void Subtract(decimal amount, string currencyCode) => Subtract(amount, currencyCode.AsSpan());
