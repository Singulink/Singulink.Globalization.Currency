...
public readonly struct Money : IFormattable, IComparable<Money>, IEquatable<Money>
{
    ...
    /// <summary>
    /// Gets the currency associated with this value or <see langword="null"/> if this is a default value with no currency associated with it.
    /// </summary>
    ...
    public Money(decimal amount, Currency? currency)
    {
        if (currency == null && amount != 0)
            throw new ArgumentException("Currency must be specified for non-zero amounts.");
...
#pragma warning restore CS1591
    ...
    /// <list type="bullet">
    ///   <item><term>"C"</term><description>Culture-Dependent International: Use currency code after the amount for English cultures, otherwise before the
    ...
    public string ToString(string? format, IFormatProvider? formatProvider = null)
    {
        ...
        if (!absAmount.TryFormat(number, out var numberLength, MemoryExtensions.TrimEnd(resultNumberFormat, default(char)), formatProvider))
            Throw.FormatEx();
        ...
        if (symbolFormat is 'I' or 'R' or 'C')
        {
            ...
            bool useReverseFormat = symbolFormat switch {
                'I' => false,
                'R' => true,
                _ => culture.TwoLetterISOLanguageName == "en",
            };

            if (useReverseFormat)
                pattern = _amount >= 0 ? PositiveReverseInternationalPattern : NegativeReverseInternationalPatterns[numberFormatInfo.CurrencyNegativePattern];
            else
                pattern = _amount >= 0 ? PositiveInternationalPattern : NegativeInternationalPatterns[numberFormatInfo.CurrencyNegativePattern];
        }
        else if (symbolFormat == 'S')
        {
            currencySymbol = _currency.Symbol;
            pattern = _amount >= 0 ? PositiveCurrencyPatterns[numberFormatInfo.CurrencyPositivePattern] : NegativeCurrencyPatterns[numberFormatInfo.CurrencyNegativePattern];
        }
        else
        {
            Throw.FormatEx();
        ...
        return ApplyPattern(number, pattern, currencySymbol, numberFormatInfo.NegativeSign);
        ...
        {
            // Up to 3 extra chars for spaces and parenthesis
            int estimatedLength = currencySymbol.Length + number.Length + negativeSign.Length + 3;
            ...
            foreach (char c in pattern)
            {
                switch (c)
                {
                    ...
                    case '-':
                        ...
                        break;
                    default:
                        sb.Append((char)160); // Non-breaking space
                        break;
                }
            }
