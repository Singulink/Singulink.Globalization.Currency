...
/// <content>
/// Contains .NET 9+ implementations for <see cref="ImmutableSortedMoneyBag"/>.
/// </content>
partial class ImmutableSortedMoneyBag
{
    ...
    public ImmutableSortedMoneyBag Add(decimal amount, string currencyCode) => Add(amount, currencyCode.AsSpan());
    ...
    public ImmutableSortedMoneyBag Remove(string currencyCode) => Remove(currencyCode.AsSpan());

    /// <inheritdoc cref="SetAmount(decimal, TargetDependentStringKey)"/>
    public ImmutableSortedMoneyBag SetAmount(decimal amount, string currencyCode) => SetAmount(amount, currencyCode.AsSpan());

    /// <inheritdoc cref="Subtract(decimal, TargetDependentStringKey)"/>
    public ImmutableSortedMoneyBag Subtract(decimal amount, string currencyCode) => Subtract(amount, currencyCode.AsSpan());
